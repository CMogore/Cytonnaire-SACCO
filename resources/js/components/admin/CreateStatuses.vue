<template>
    <div v-if="isOpen" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white p-8 rounded-lg max-w-md">
        ...
        <form @submit.prevent="userData ? updateStatus() : addStatus()">
          <div class="mb-4">
  ...
  export default {
    ...
    addStatus() {
      ...
      axios.post('/api/admin/createstatus', this.formData, {
    ...
    updateStatus() {
      const authToken = localStorage.getItem('auth_token');
      axios.put(`/api/admin/status/${this.userData.id}`, this.formData, {
        headers: { 'Authorization': `Bearer ${authToken}` }
      })
      .then(response => {
        this.$emit('saveUser'); // Emit event to refresh user list
